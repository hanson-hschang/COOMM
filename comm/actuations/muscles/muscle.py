...
@njit(cache=True)
def force_length_weight_poly(muscle_length, f_l_coefficients=np.array([-6.44, 18.01, -13.64, 3.06])):
...
class Muscle(ContinuousActuation, MuscleInfo):
    ...
    def __init__(self, ratio_muscle_position, rest_muscle_area, **kwargs):
        ...
        MuscleInfo.__init__(self,
            type_name=kwargs.get("type_name", "muscle"),
    ...
    def __call__(self, system):
        ...
        self.calculate_muscle_strain(
            ...
            system.sigma, system.kappa,
            system.rest_voronoi_lengths,
            system.voronoi_dilatation
        )
        self.calculate_muscle_tangent(
            self.muscle_tangent, self.muscle_strain
        )
    ...
    @staticmethod
    @njit(cache=True)
    def calculate_muscle_area(
        rest_muscle_area, muscle_area, dilatation
    ):
        muscle_area[:] = rest_muscle_area / dilatation

    @staticmethod
    @njit(cache=True)
    def calculate_muscle_position(
    ...
    def calculate_muscle_strain(
        muscle_strain, 
        off_center_displacement, sigma, kappa, 
        rest_voronoi_lengths, voronoi_dilatation
    ):
        shear = sigma_to_shear(sigma)
        muscle_position_derivative = (
            difference2D(off_center_displacement) / 
                (rest_voronoi_lengths * voronoi_dilatation)
        )
        muscle_strain[:, :] = shear + quadrature_kernel(
            _batch_cross(
                kappa, average2D(off_center_displacement)
            ) + muscle_position_derivative
        )

    @staticmethod
    @njit(cache=True)
    def calculate_muscle_tangent(
        muscle_tangent, muscle_strain
    ):  
    ...
    def calculate_muscle_normalized_length(
        ...
        muscle_normalized_length[:] = (
            muscle_length / muscle_rest_length
        )
...
class MuscleForce(Muscle):
    ...
    def __call__(self, system):
        ...
        self.calculate_muscle_normalized_length(
            ...
            self.muscle_length, self.muscle_rest_length
        )
        ...
        self.calculate_force_and_couple(
            self.muscle_force, self.muscle_tangent, 
            self.muscle_position,
            self.internal_force, self.internal_couple,
            self.external_force, self.external_couple,
    ...
    def calculate_muscle_force(
        muscle_force, muscle_activation, max_muscle_stress, 
        weight, muscle_area
    ):
        muscle_force[:] = (
            muscle_activation * max_muscle_stress * weight
        ) * muscle_area

    @staticmethod
    @njit(cache=True)
    def calculate_force_and_couple(
        muscle_force, muscle_tangent, 
        muscle_position,
        ...
        _force_induced_couple(
            internal_force, muscle_position, internal_couple
        )
        _internal_to_external_load(
            director_collection, kappa, tangents,
            rest_lengths, rest_voronoi_lengths,
...
class MuscleGroup(ContinuousActuation, MuscleInfo):
    ...
    def __call__(self, system):
        ...
        for muscle in self.muscles:
            muscle(system)
            inplace_addition(
                self.internal_force, muscle.internal_force
            )
            inplace_addition(
                self.external_force, muscle.external_force
            )
            inplace_addition(
                self.internal_couple, muscle.internal_couple
            )
            inplace_addition(
                self.external_couple, muscle.external_couple
            )
...
class ApplyMuscles(ApplyActuations):
    ...
    def callback_func(self, muscles, callback_params_list):
        ...
        for muscle, callback_params in zip(
            ...
            callback_params['muscle_info'].append(
                str(muscle)
            )
            callback_params['s_activation'].append(
                muscle.s_activation.copy()
            )
            callback_params['activation'].append(
                muscle.activation.copy()
            )
            callback_params['muscle_length'].append(
                muscle.muscle_length.copy()
            )
            ...
            callback_params['force_length_weight'].append(
                muscle.force_length_weight(
                    muscle.muscle_normalized_length
                
                ).copy()
            )
            callback_params['muscle_position'].append(
                muscle.muscle_position.copy()
            )
            callback_params['internal_force'].append(
                muscle.internal_force.copy()
            )
            callback_params['internal_couple'].append(
                muscle.internal_couple.copy()
            )
            callback_params['external_force'].append(
                muscle.external_force.copy()
            )
            callback_params['external_couple'].append(
                muscle.external_couple.copy()
            )
...
class ApplyMuscleGroups(ApplyMuscles):
    ...
    def callback_func(self, muscle_groups, callback_params_list):
        ...
        for muscle_group, callback_params in zip(
            ...
            callback_params['muscle_group_info'].append(
                str(muscle_group)
            )
            callback_params['s_activation'].append(
                muscle_group.s_activation.copy()
            )
            callback_params['activation'].append(
                muscle_group.activation.copy()
            )
            callback_params['internal_force'].append(
                muscle_group.internal_force.copy()
            )
            callback_params['internal_couple'].append(
                muscle_group.internal_couple.copy()
            )
            callback_params['external_force'].append(
                muscle_group.external_force.copy()
            )
            callback_params['external_couple'].append(
