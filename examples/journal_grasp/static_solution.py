...
def get_algo(rod, muscles, target):
    ...
    algo = ForwardBackwardMuscle(
        ...
        object=CylinderTarget.get_cylinder(
            cylinder=target,
            n_elements=100,
            cost_weight=dict(position=1e7 * np.ones(s.shape), director=0),
            target_cost_weight=dict(position=1e6 * weight, director=1e3 * weight),
        ),
    )
    # director = np.eye(3)
...
def main(filename):
    ...
    algo = get_algo(rod=systems[0], muscles=env.muscle_groups, target=systems[1])
    ...
    ax_main.scatter(base[0], base[1], base[2], color="grey")
    cylinder = systems[1]
    ...
    for iter_number in tqdm(range(max_iter_number)):
        ...
        if algo.iteration in iteration_list:
            ...
            ax_main.scatter(base[0], base[1], base[2], color="red")
            for i in range(3):
    ...
    frame.plot_strain(
        shear=algo.static_rod.sigma + np.array([0, 0, 1])[:, None], kappa=algo.static_rod.kappa
    )
