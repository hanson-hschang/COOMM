...
class Frame(RodFrame, StrainFrame):
    ...
    def get_frame(cls, filename):
        ...
        axes_strain_info = dict(
            axes_kappa_indices=[[0, 5], [1, 5], [2, 5]], axes_shear_indices=[[0, 4], [1, 4], [2, 4]]
        )
        ax_main_info = dict(indices=[slice(0, 3), slice(0, 3)], planner_flag=False)
...
def main(filename):
    ...
    for k in tqdm(range(len(rod_data["time"]))):
        ...
        axes_shear, axes_curvature = frame.plot_strain(
            shear=rod_data["sigma"][k] + np.array([0, 0, 1])[:, None], kappa=rod_data["kappa"][k]
        )
    ...
    frame.movie(frame_rate=recording_fps, movie_name=filename + "_movie")


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Plot simulation result in frames.")
    parser.add_argument(
        "--filename",
        type=str,
        default="simulation",
