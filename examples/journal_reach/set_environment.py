...
class Environment(ArmEnvironment):
    ...
    def set_target(self):
        ...
        self.sphere = el.Sphere(
            center=np.array([0.01, 0.15, 0.06]), base_radius=target_radius, density=1000
        )
        self.sphere.director_collection[:, :, 0] = np.array([[0, 0, 1], [0, 1, 0], [-1, 0, 0]])
        ...
        self.simulator.collect_diagnostics(self.sphere).using(
            SphereCallBack, step_skip=self.step_skip, callback_params=self.sphere_parameters_dict
        )
