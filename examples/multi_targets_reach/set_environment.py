...
class Environment(ArmEnvironment):
    ...
    def set_target(self):
        ...
        sphere_directors = [
            ...
            np.array(
                [[0, 0, 1], [np.cos(angle), np.sin(angle), 0], [-np.sin(angle), np.cos(angle), 0]]
            ),
        ]

        """ Set up a sphere object """
        for position, director in zip(sphere_positions, sphere_directors):
            sphere = el.Sphere(center=position, base_radius=0.006, density=1000)
            ...
            self.simulator.collect_diagnostics(sphere).using(
                SphereCallBack, step_skip=self.step_skip, callback_params=sphere_parameters_dict
            )
