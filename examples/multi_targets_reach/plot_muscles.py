...
from coomm.frames.muscle_frame import muscle_length_color, muscle_force_weight_color, muscles_color
...
class Frame(RodFrame, TransverseMuscleFrame, LongitudinalMuscleFrame, ObliqueMuscleFrame):
    ...
    def reset(
        ...
        MuscleFrameBase.reset(
            self,
        )
        TransverseMuscleFrame.reset(
            self,
        )
        LongitudinalMuscleFrame.reset(
            self,
        )
        ObliqueMuscleFrame.reset(
            self,
        )
        RodFrame.reset(
            self,
    ...
    def set_labels(self, time=None):
        ...
        self.axes_TM["muscle"][0].set_title("TM", color=muscles_color["TM"], fontsize=self.fontsize)
        self.axes_LM["muscle"][0].set_title(
            r"LM$_{1^+}$", color=muscles_color["LM"], fontsize=self.fontsize
        )
        self.axes_LM["muscle"][1].set_title(
            r"LM$_{2^+}$", color=muscles_color["LM"], fontsize=self.fontsize
        )
        ...
        self.axes_OM["muscle"][0].set_ylabel(
            r"OM$_+$", color=muscles_color["OM"], fontsize=self.fontsize
        )
    ...
    def plot_muscle_activation(
        self, group_number, muscle_group_type, muscle_s_activation, muscle_activation, **kwargs
    ):
        axes_muscle = getattr(self, "axes_" + muscle_group_type)
    ...
    def plot_muscle_length(self, group_number, muscle_type, muscle_number, muscle_s, muscle_length):
        axes_muscle = getattr(self, "axes_" + muscle_type)
        return MuscleFrameBase.plot_muscle_length(
            axes_muscle["length"][group_number][muscle_number], muscle_s, muscle_length
        )

    def plot_force_length_weight(
        self, group_number, muscle_type, muscle_number, muscle_s, force_length_weight
    ):
        axes_muscle = getattr(self, "axes_" + muscle_type)
        return MuscleFrameBase.plot_force_weight(
            axes_muscle["force_weight"][group_number][muscle_number], muscle_s, force_length_weight
        )
    ...
    def get_frame(cls, filename):
        ...
        ax_muscles_info = dict(indices=[slice(0, 4), slice(5, 10)])
        ...
        axes_TM_info = dict(
            indices=[
                dict(activation=[0, 5], others=[[1, 5]], muscle=[slice(0, 2), 5]),
            ],
            ...
            axes_muscle_offset=[[ax_muscle_p0_offset, [0, ax_muscle_y1_offset]]],
        )

        axes_LM_info = dict(
            indices=[
                dict(activation=[0, 6], others=[[1, 6]], muscle=[slice(0, 2), 6]),
                dict(activation=[0, 7], others=[[1, 7]], muscle=[slice(0, 2), 7]),
                dict(activation=[0, 8], others=[[1, 8]], muscle=[slice(0, 2), 8]),
                dict(activation=[0, 9], others=[[1, 9]], muscle=[slice(0, 2), 9]),
            ],
            yticklabels_showflag=[
                dict(
                    activation=True,
        ...
        ax_main_info = dict(indices=[slice(0, 4), slice(0, 4)], planner_flag=False)
...
def main(filename):
    ...
    for k in tqdm(range(len(rod_data["time"]))):
        ...
        for muscle_group_data in muscle_groups_data:
            ...
            group_number, muscle_group_type = (int(muscle_info[0]), muscle_info[1])
            ...
            for muscle_data in muscle_group_data["muscles"]:
                ...
                muscle_number, muscle_type = (int(muscle_info[0]), muscle_info[1])
                frame.plot_muscle_length(
                    group_number,
                    muscle_type,
                    muscle_number,
                    muscle_data["s_activation"][k],
                    muscle_data["muscle_normalized_length"][k],
                )
                frame.plot_force_length_weight(
                    ...
                    muscle_number,
                    muscle_data["s_activation"][k],
                    muscle_data["force_length_weight"][k],
                )

        frame.set_ax_main_lim(x_lim=[-1.1, 1.1], y_lim=[-1.1, 1.1], z_lim=[-1.1, 1.1])

        frame.set_labels(rod_data["time"][k])
        frame.save()

    frame.movie(frame_rate=recording_fps, movie_name=filename + "_muscle_movie")
