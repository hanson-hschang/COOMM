...
class DragForce(NoForces):
    ...
    def __init__(
        self, rho_environment, c_per, c_tan, system, step_skip: int, callback_params: dict
    ):
        """__init__.
    ...
    def callback_func(self):
        """callback_func."""
        self.callback_params["velocity_material_frame"].append(self.velocity_material_frame.copy())
    ...
    def calculate_drag_force(
        ...
        velocity_material_frame[:, :] = _lab_to_material(director, average2D(velocity))
        square_velocity_with_direction = np.abs(velocity_material_frame) * velocity_material_frame
        drag_force_material_frame[:2, :] = -scale_per * square_velocity_with_direction[:2, :]
        drag_force_material_frame[2, :] = -scale_tan * square_velocity_with_direction[2, :]
        drag_force[:, :] = quadrature_kernel(_material_to_lab(director, drag_force_material_frame))
