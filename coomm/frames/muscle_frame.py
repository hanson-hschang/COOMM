...
class MuscleFrameBase(FrameBase):
    ...
    def __init__(self, file_dict, fig_dict, gs_dict, **kwargs):
        ...
        FrameBase.__init__(  # Try to use super
            self, file_dict=file_dict, fig_dict=fig_dict, gs_dict=gs_dict
        )
    ...
    def ax_muscles_reset(self):
        ...
        ax = self.fig.add_subplot(
            self.gs[self.ax_muscles_info["indices"][0], self.ax_muscles_info["indices"][1]]
        )
    ...
    def reset(self, axes_muscle_info=None):
        ...
        for i, axes_muscle_indices_i in enumerate(axes_muscle_info["indices"]):
            ...
            ax = self.fig.add_subplot(
                self.gs[
                    axes_muscle_indices_i["activation"][0], axes_muscle_indices_i["activation"][1]
                ],
                xlim=[-0.05, 1.15],
                ylim=[-0.1, 1.1],
            )
            ax = change_box_to_arrow_axes(self.fig, ax, xaxis_ypos=-0.05, x_offset=[0.05, -0.05])
            ...
            for j, axes_muscle_indices_i_j in enumerate(axes_muscle_indices_i["others"]):
                ax = self.fig.add_subplot(
                    self.gs[axes_muscle_indices_i_j[0], axes_muscle_indices_i_j[1]],
                    xlim=[-0.05, 1.15],
                    ylim=[-0.2, 2.2],
                )
                ax = change_box_to_only_x_line_ax(
                    self.fig, ax, xaxis_ypos=-0.1, x_offset=[0.05, -0.15]
                )
                ax = change_box_to_only_y_arrow_ax(
                    self.fig, ax, yaxis_xpos=0.0, y_offset=[0.1, 0], color=muscle_length_color
                )
                ax.tick_params(axis="x", direction="in", length=4, width=1, pad=0)
                ax.tick_params(
                    axis="y", direction="in", length=4, width=1, pad=1, colors=muscle_length_color
                )
            ...
            ax = self.fig.add_subplot(
                self.gs[axes_muscle_indices_i["muscle"][0], axes_muscle_indices_i["muscle"][1]]
            )
    ...
    def plot_muscle_length(cls, ax, s, muscle_length, **kwargs):
        ax.plot(s, muscle_length, color=kwargs.get("color", muscle_length_color))
        return ax

    @classmethod
    def plot_force_weight(cls, ax, s, force_weight, **kwargs):
        ax.plot(s, force_weight, color=kwargs.get("color", muscle_force_weight_color))
        return ax
...
class TransverseMuscleFrame(MuscleFrameBase):
    ...
    def __init__(self, file_dict, fig_dict, gs_dict, **kwargs):
        ...
        MuscleFrameBase.__init__(
            self, file_dict=file_dict, fig_dict=fig_dict, gs_dict=gs_dict, **kwargs
        )
