...
class RodFrame(FrameBase):
    ...
    def __init__(self, file_dict, fig_dict, gs_dict, **kwargs):
        ...
        gs_dict :
        """
        FrameBase.__init__(self, file_dict=file_dict, fig_dict=fig_dict, gs_dict=gs_dict)
        ...
        self.plot_rod = self.plot_rod3d if self.ax_main_3d_flag else self.plot_rod2d
    ...
    def reset(
        ...
        if self.ax_main_3d_flag:
            self.ax_main = self.fig.add_subplot(
                self.gs[self.ax_main_indices[0], self.ax_main_indices[1]], projection="3d"
            )
        else:
            self.ax_main = self.fig.add_subplot(
                self.gs[self.ax_main_indices[0], self.ax_main_indices[1]]
            )
    ...
    def set_ref_configuration(self, position):
        ...
        reference_length = np.linalg.norm(position[:, 1:] - position[:, :-1], axis=0)
    ...
    def plot_ref_configuration(
        ...
        line_position = (
            process_position(self.reference_position, self.offset, self.rotation)
            / self.reference_total_length
        )

        if self.ax_main_3d_flag:
            self.ax_main.plot(
                line_position[0], line_position[1], line_position[2], color="grey", linestyle="--"
            )
        else:
            self.ax_main.plot(line_position[0], line_position[1], color="grey", linestyle="--")
    ...
    def calculate_line_position(self, position, director, radius):
        ...
        """
        line_center = (
            process_position(position, self.offset, self.rotation) / self.reference_total_length
        )
        line_position = process_position(
            (position[:, :-1] + position[:, 1:]) / 2, self.offset, self.rotation
        )
        line_director = process_director(director, self.rotation)
        line_up = (line_position + line_director[1, :, :] * radius) / self.reference_total_length
        line_down = (line_position - line_director[1, :, :] * radius) / self.reference_total_length
        line_left = (line_position + line_director[0, :, :] * radius) / self.reference_total_length
        line_right = (line_position - line_director[0, :, :] * radius) / self.reference_total_length
    ...
    def plot_rod2d(self, position, director, radius, **kwargs):
        ...
        line_center, lines = self.calculate_line_position(position, director, radius)
        self.ax_main.plot(line_center[0], line_center[1], color=color, alpha=alpha, linestyle="--")
        self.ax_main.plot(
            lines[0][0],
            lines[0][1],
            color=color,
    ...
    def plot_rod3d(self, position, director, radius, **kwargs):
        ...
        line_center, lines = self.calculate_line_position(position, director, radius)
        self.ax_main.plot(
            line_center[0], line_center[1], line_center[2], color=color, alpha=alpha, linestyle="--"
        )
        for line in lines:
            self.ax_main.plot(
                line[0],
    ...
    def set_labels(self, time=None):
        ...
        """
        if time is not None:
            self.ax_main.set_title("time={:.2f} [sec]".format(time), fontsize=self.fontsize)
...
class StrainFrame(FrameBase):
        ...
        for i in range(3):
            self.axes_kappa.append(
                self.fig.add_subplot(
                    self.gs[self.axes_kappa_indices[i][0], self.axes_kappa_indices[i][1]],
                    xlim=[-0.1, 1.1],
                )
            )
            self.axes_shear.append(
                self.fig.add_subplot(
                    self.gs[self.axes_shear_indices[i][0], self.axes_shear_indices[i][1]],
                    xlim=[-0.1, 1.1],
                )
            )

        if self.reference_configuration_flag:
            StrainFrame.plot_ref_configuration(
                self,
            )
    ...
    def plot_ref_configuration(
        ...
        for index_i in range(3):
            self.axes_shear[index_i].plot(
                self.s_shear, self.reference_shear[index_i], color="grey", linestyle="--"
            )
            self.axes_kappa[index_i].plot(
                self.s_kappa, self.reference_kappa[index_i], color="grey", linestyle="--"
            )
    ...
    def plot_strain(self, shear, kappa, color=None):
        ...
        for index_i in range(3):
            self.axes_shear[index_i].plot(
                self.s_shear, shear[index_i], color=self.rod_color if color is None else color
            )
            self.axes_kappa[index_i].plot(
                self.s_kappa, kappa[index_i], color=self.rod_color if color is None else color
            )
    ...
    def set_axes_strain_lim(
        ...
        for index_i in range(3):
            ...
            kappa_log = np.floor(np.log10(axes_kappa_lim[index_i][1] - kappa_mean))
            ...
            self.axes_kappa[index_i].ticklabel_format(
                axis="y", scilimits=(kappa_log, kappa_log), useOffset=kappa_mean
            )
    ...
    def set_labels(
        ...
        for index_i in range(3):
            self.axes_shear[index_i].set_ylabel(
                "d$_{}$".format(index_i + 1), fontsize=self.fontsize, rotation=0, labelpad=15.0
            )
        ...
        ylim_mean = np.average(ylim)
        position = 0.9 * (ylim[1] - ylim_mean) + ylim_mean
        self.axes_kappa[2].text(1, position, "twist", fontsize=self.fontsize, ha="right", va="top")
